...
### buffer:
- **Descripción**: The buffer to record into. Must either be a buffer object, buffer name, list of buffer name and size or the buffer id as a number.
- **Default**: 
- **Restricciones**: must be a buffer description. such as a buffer, :foo, "foo", or [:foo, 4]
